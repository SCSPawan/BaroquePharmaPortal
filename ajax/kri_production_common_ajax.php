...
if (isset($_POST['SubITFG_Btn'])) {
	...
	$tdata['Series'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_Series'])));
	$tdata['DocDate'] = date("Y-m-d", strtotime($_POST['TransferToUndertest_PostingDate']));
	$tdata['DueDate'] = date("Y-m-d", strtotime($_POST['TransferToUndertest_DocumentDate']));
	$tdata['CardCode'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_SupplierCode'])));
	$tdata['Comments'] = null;
	$tdata['FromWarehouse'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_i_FromWhs'])));
	$tdata['ToWarehouse'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_i_ToWhs'])));
	$tdata['TaxDate'] = date("Y-m-d", strtotime($_POST['TransferToUndertest_DocumentDate']));
	$tdata['DocObjectCode'] = trim(addslashes(strip_tags('67')));
	$tdata['BPLID'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_BPL_Id'])));
	$tdata['U_PC_SIIProc'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_DocEntry'])));
	$tdata['U_BFType'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_BaseDocType'])));

	$mainArray = $tdata;
	// --------------------- Item and batch row data preparing start here -------------------------------- -->
	$item['LineNum'] = trim(addslashes(strip_tags('0')));
	$item['ItemCode'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_i_ItemCode'])));
	$item['WarehouseCode'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_i_ToWhs'])));
	$item['FromWarehouseCode'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_i_FromWhs'])));
	$item['Quantity'] = trim(addslashes(strip_tags($_POST['TransferToUndertest_i_BQty'])));
...
if (isset($_POST['action']) && $_POST['action'] == 'OpenInventoryTransferSamplessue_stability_ajax') {
    ...
    // <!-- --------- Item HTML Table Body Prepare Start Here ------------------------------ -->
        if (!empty($response)) {
            $FinalResponce['option'] = '<tr>
                <td class="desabled">
                    <input type="hidden" id="it_DocEntry" name="it_DocEntry" value="' . $response[0]->DocEntry . '">
                    <input type="hidden" id="it_BatchNo" name="it_BatchNo" value="' . $response[0]->BatchNo . '">
                    1
                </td>
    ...
    // <!-- --------- Container Selection Start Here --------------------------------------- -->
        ...
        // <!-- --------- Container Selection HTML Table Body Prepare Start Here ------------------------------ --> 
            if (!empty($response_container)) {
                for ($i = 0; $i < count($response_container); $i++) {
                    // ----------- Date formating condition definr start here---------------------------
                        $MfgDate =(!empty($response_container[$i]->MfgDate)) ? date("d-m-Y", strtotime($response_container[$i]->MfgDate)) : '';

                        $ExpiryDate =(!empty($response_container[$i]->ExpDate)) ? date("d-m-Y", strtotime($response_container[$i]->ExpDate)) : '';
                    // ----------- Date formating condition definr end here-----------------------------
        ...
        // <!-- --------- Container Selection HTML Table Body Prepare End Here -------------------------------- --> 
    // <!-- --------- Container Selection End Here ----------------------------------------- -->
    echo json_encode($FinalResponce);
    exit(0);
}
...
if (isset($_POST['SubIT_Btn_S_sample_issue_sampleCollection_stability'])) {
	...
	$tdata['Series'] = trim(addslashes(strip_tags($_POST['stability_SeriesName'])));
		...
		if (!empty($res)) {
			...
			//  <!-- ------- service layer function responce manage Start Here ------------ -->
				...
				if (array_key_exists('error', (array)$responce)) {
					...
					$underTestNumber = $obj->PATCH_ServiceLayerMasterFunctionWithB1Replace($UT_data, $Final_API2);
					$underTestNumber_decode = json_decode($underTestNumber);

					if (empty($underTestNumber_decode)) {
						$data['status'] = 'True';
						$data['DocEntry'] = $responce->DocEntry;
						$data['message'] = "Goods Issue Transfer Successfully Added.";
						echo json_encode($data);
					} else {
						if (array_key_exists('error', (array)$underTestNumber_decode)) {
							$data['status'] = 'False';
...
if (isset($_POST['SubIT_Btn_post_extra_issue_fg'])) {
	...
	$tdata['BPL_IDAssignedToInvoice'] = trim(addslashes(strip_tags($_POST['fg_it_BPLID'])));
	$tdata['U_PC_SCFG'] = trim(addslashes(strip_tags($_POST['fg_it_RcDocEntry'])));
	...
	// die();
	// // --------------------- Item and batch row data preparing start here   -------------------------------- -->
	$item['BaseLine'] = trim(addslashes(strip_tags('0')));
	$item['ItemCode'] = trim(addslashes(strip_tags($_POST['itP_ItemCode'])));
	$item['Quantity'] = trim(addslashes(strip_tags($_POST['itP_BQty'])));
	// $item['BaseType'] = trim(addslashes(strip_tags($_POST['invtr_Extra_doctyp'])));
	// $item['BaseEntry'] = trim(addslashes(strip_tags($_POST['fg_it_DocEntry'])));
...
// exit;
	...
	for ($i = 0; $i < count($_POST['usercheckList_extra']); $i++) {
		if ($_POST['usercheckList_extra'][$i] == '1') {
			$batch['BatchNumber'] = trim(addslashes(strip_tags($_POST['itp_Batch_extra'][$i])));
			$batch['Quantity'] = trim(addslashes(strip_tags($_POST['SelectedQty_extra'][$i])));
			$batch['BaseLineNumber'] = trim(addslashes(strip_tags('0')));
			$batch['ItemCode'] = trim(addslashes(strip_tags($_POST['itp_ItemCode_extra'][$i])));
			$item['BatchNumbers'][] = $batch;
...
// exit;
	...
	if (!empty($res)) {
		...
		if (array_key_exists('error', (array)$responce)) {
			...
			$UT_data = [
				'DocEntry' => $_POST['fg_it_RcDocEntry'],
				'SCS_SCOLFG2Collection' => [
					[
						'LineId' => $_POST['it_Linenum'],
						'U_PC_PEIs' => $responce->DocEntry
					]
				]
			];
			...
			// <!-- ------- row data preparing end here ----------------------- -->
			// SCS_SCINPROCESS_SL
			$Final_API2 = $SAP_URL . ":" . $SAP_Port . "/b1s/v1/" . $SCS_SCOLFG_API . '(' . $_POST['fg_it_RcDocEntry'] . ')';
			$underTestNumber = $obj->SampleIntimationUnderTestUpdateFromInventoryTransfer($UT_data, $Final_API2);
			$underTestNumber_decode = json_decode($underTestNumber);
...
if (isset($_POST['updateQcPostDocumentStabilitytBtn'])) {
	...
	//   exit;
	$tdata = array(); // This array send to AP Standalone Invoice process 
	$tdata['Series'] = trim(addslashes(strip_tags($_POST['StabilityQC_CK_D_DocName'])));
	$tdata['Object'] = 'SCS_QCSTAB';
	$tdata['U_PC_BPLId'] = trim(addslashes(strip_tags($_POST['Stability_BPLId'])));
	$tdata['U_PC_LocCode'] = trim(addslashes(strip_tags($_POST['Stability_QC_LOC'])));
	$tdata['U_PC_Loc'] = trim(addslashes(strip_tags($_POST['Location'])));
	$tdata['U_PC_RNo'] = trim(addslashes(strip_tags($_POST['ReceiptNo'])));
	$tdata['U_PC_REnt'] = trim(addslashes(strip_tags($_POST['ReceiptEntry'])));
	$tdata['U_PC_WoNo'] = trim(addslashes(strip_tags($_POST['WoNo'])));
	$tdata['U_PC_WoEnt'] = trim(addslashes(strip_tags($_POST['WoEntry'])));
	$tdata['U_PC_ICode'] = trim(addslashes(strip_tags($_POST['ItemCode'])));
	$tdata['U_PC_IName'] = trim(addslashes(strip_tags($_POST['ItemName'])));
	$tdata['U_PC_GName'] = trim(addslashes(strip_tags($_POST['GenericName'])));
	$tdata['U_PC_LClaim'] = trim(addslashes(strip_tags($_POST['LabelCliam'])));
	...
	// $tdata['U_PC_SType'] = trim(addslashes(strip_tags($_POST['Stability'])));
	$tdata['U_PC_SType'] = trim(addslashes(strip_tags($_POST['SampleType'])));
	$tdata['U_PC_BNo'] = trim(addslashes(strip_tags($_POST['BatchNo'])));
	$tdata['U_PC_BSize'] = trim(addslashes(strip_tags($_POST['BatchSize'])));
	$tdata['U_PC_MfgDt'] = trim(addslashes(strip_tags($_POST['MfgDate'])));
	$tdata['U_PC_ExpDt'] = trim(addslashes(strip_tags($_POST['ExpiryDate'])));
	$tdata['U_PC_SIntNo'] = trim(addslashes(strip_tags($_POST['SampleIntimationStability'])));
	$tdata['U_PC_SColNo'] = trim(addslashes(strip_tags($_POST['SampleCollStability'])));
	$tdata['U_PC_SQty'] = 0;
	$tdata['U_PC_RQty'] = 0;
	$tdata['U_PC_PckSize'] = trim(addslashes(strip_tags($_POST['PackSize'])));
	$tdata['U_PC_SamType'] = "Regular";
	...
	$tdata['U_PC_ChkBy'] = trim(addslashes(strip_tags($_POST['StabilityQC_CK_D_CheckedBy'])));
	$tdata['U_PC_AnlBy'] = trim(addslashes(strip_tags($_POST['StabilityQC_CK_D_AnalysisBy'])));
	$tdata['U_PC_Remarks'] = trim(addslashes(strip_tags($_POST['StabilityQC_CK_D_Remarks'])));
	$tdata['U_PC_AsyCal'] = trim(addslashes(strip_tags($_POST['QC_CK_D_Assay'])));
	...
	$tdata['U_PC_StType'] = trim(addslashes(strip_tags($_POST['SampleType'])));
	...
	$tdata['U_PC_AnType'] = trim(addslashes(strip_tags($_POST['AnalysisType'])));
	$tdata['U_PC_WhsCode'] = trim(addslashes(strip_tags($_POST['WhsCode'])));
	$tdata['U_PC_BEnt'] = trim(addslashes(strip_tags($_POST['SampleCollEntryFromWO'])));
	$tdata['U_PC_BNum'] = trim(addslashes(strip_tags($_POST['SampleTransferNoFromWO'])));
	$tdata['U_PC_StDNo'] = trim(addslashes(strip_tags($_POST['StabilityPlanDocNum'])));
	$tdata['U_PC_StDEnt'] = trim(addslashes(strip_tags($_POST['StabilityPlanDocEntry'])));
	$tdata['U_PC_StDt'] = trim(addslashes(strip_tags($_POST['StabilityLoadingDate'])));
	...
	$tdata['U_PC_LClmUom'] = trim(addslashes(strip_tags($_POST['Stability_QC_UOM'])));
	$ganaralData = array();
	// $BL = 0;
	for ($i = 0; $i < count($_POST['parameter_code']); $i++) {
		$ganaralData['LineId'] = ($i + 1);
...
if (isset($_POST['action']) && $_POST['action'] == 'QCPostdocumentQCPost_Stability_row') {
	...
	if (!empty($general_data)) {
		for ($i = 0; $i < count($general_data); $i++) {
			...
			if ($general_data[$i]->PDType == 'Range') {
				$FinalResponce['general_data'] .= '<td>
				<input type="text" id="ComparisonResult' . $SrNo . '" name="ComparisonResult[]" value="' . $general_data[$i]->CResult . '" class="form-control" style="width:100px;" onfocusout="CalculateResultOut(' . $SrNo . ')">
			</td>';
			} else {
				$FinalResponce['general_data'] .= '<td class="desabled">
			...
			<td><input  type="text" id="analyst_remark' . $SrNo . '" name="analyst_remark[]" class="form-control" value="' . $general_data[$i]->AnalyRemarks . '"></td>
			...
			<td><input  type="number" id="UppMinRes' . $SrNo . '" name="UppMinRes[]" class="form-control"  value="' . $general_data[$i]->UppMin1 . '"  ></td>
			
			<td><input  type="number" id="upper_max_result' . $SrNo . '" name="upper_max_result[]" class="form-control" value="' . $general_data[$i]->UppMaxResult . '"></td>

			<td>
				<input type="number" id="MeanRes' . $SrNo . '" name="MeanRes[]" class="form-control" value="' . $general_data[$i]->MeanResult . '">
			</td>
...
if (isset($_POST['addQcPostDocumentSubmitQCCheckBtnStability'])) {
	...
	$tdata['U_PC_REnt'] = trim(addslashes(strip_tags($_POST['stability_ReceiptEntry'])));
	...
	$tdata['U_PC_RfBy'] = null;
	$tdata['U_PC_SType'] = trim(addslashes(strip_tags($_POST['stability_StabilityType'])));
	...
	$tdata['U_PC_StCon'] = trim(addslashes(strip_tags($_POST['stability_StabilityCondition'])));
	$tdata['U_PC_StTPer'] = trim(addslashes(strip_tags($_POST['stability_StabilityTimePeriod'])));
	$tdata['U_PC_AnType'] = trim(addslashes(strip_tags($_POST['stability_AnalysisType'])));
	$tdata['U_PC_WhsCode'] = trim(addslashes(strip_tags($_POST['stability_WhsCode'])));
	$tdata['U_PC_BEnt'] = trim(addslashes(strip_tags($_POST['stability_StabilityTransferNofromWO'])));
	$tdata['U_PC_BNum'] = trim(addslashes(strip_tags($_POST['stability_StabilityTransferEntryfromWO'])));
	$tdata['U_PC_StDNo'] = trim(addslashes(strip_tags($_POST['stability_StabilityPlanDocNum'])));
	$tdata['U_PC_StDEnt'] = trim(addslashes(strip_tags($_POST['StabilityPlanDocEntry'])));
	...
	for ($i = 0; $i < count($_POST['parameter_code']); $i++) {
		...
		$ganaralData['U_PC_InCod'] = trim(addslashes(strip_tags($_POST['InstrumentCode'][$i])));
		$ganaralData['U_PC_InNam'] = trim(addslashes(strip_tags($_POST['InstrumentName'][$i])));
		$ganaralData['U_PC_SDt'] = trim(addslashes(strip_tags($_POST['start_date'][$i])));
		$ganaralData['U_PC_STime'] = trim(addslashes(strip_tags($_POST['start_time'][$i])));
		$ganaralData['U_PC_EDate'] = trim(addslashes(strip_tags($_POST['end_date'][$i])));
		$ganaralData['U_PC_ETime'] = trim(addslashes(strip_tags($_POST['end_time'][$i])));
		$tdata['SCS_QCSTAB1Collection'][] = $ganaralData; // row data append on this array
