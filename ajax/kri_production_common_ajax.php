...
if(isset($_POST['action']) && $_POST['action'] =='getSeriesSingleData_ajax')
{
	...
	// $Final_API=$INWARDQCSERIES.$ObjectCode.'&Series='.$Series;
	// // print_r($Final_API);die();
	// $response=$obj->GetSeriesSingleData($Final_API);
	echo json_encode($response);
	exit(0);
}
...
if(isset($_POST['action']) && $_POST['action'] =='sample_collecton_in_process_ajax')
{
    ...
    // print_r(count($ExternalIssue));die();
...
// <!-- ----------- External Issue Start Here ---------------------------- -->
		if(!empty($ExternalIssue)){
			for ($j=0; $j <count($ExternalIssue) ; $j++) { 
				...
				$FinalResponce['ExternalIssue'].='<tr id="'.$SrNo.'">
...
if(isset($_POST['action']) && $_POST['action'] =='getSempleCSeriesDropdown_ajax')
{
	...
	$Final_API=$INWARDQCSERIES.$ObjectCode;
	$response=$obj->GetSeriesDropdown($Final_API);
	echo json_encode($response);
	exit(0);
}
...
if(isset($_POST['SampleCollectionProcessInUpdateForm_Btn']))
{
	// <!-- ------------ array declare Here ------------- -->
		...
		// exit;
	// <!-- ------------ array declare Here ------------- -->
		$tdata['Series']=trim(addslashes(strip_tags($_POST['numner_Series'])));
		...
		if(!empty($_POST['DateofReversal'])){
			$tdata['U_PC_DRev']=trim(addslashes(strip_tags($_POST['DateofReversal'])));
		}else{
			$tdata['U_PC_DRev']=null;
		}

		 if(!empty($_POST['SampleIssue'])){
			$tdata['U_PC_SIssue']=trim(addslashes(strip_tags($_POST['SampleIssue'])));
		}else{
			$tdata['U_PC_SIssue']=null;
		}

		if(!empty($_POST['ReverseSampleIssue'])){
			$tdata['U_PC_RSIssue']=trim(addslashes(strip_tags($_POST['ReverseSampleIssue'])));
		}else{
...
// .'&ItemCode='.$_POST['ItemCode'].'&LineNum='.$_POST['LineNum']
   ...
   // 	exit;
	...
	// <!-- ------ Array declaration End Here  --------------------------------- -->
       $FinalResponce['SampleCollDetails']=$response;
	  $general_data=$response[0]->RETESTQCPOSTROWDETAILS; 
...
if(isset($_POST['action']) && $_POST['action'] =='Open_Transaction_For_Sample_collection_in_process_popup_in_process')  // API Ser No 40 somthing wrong
{
...
if(isset($_POST['action']) && $_POST['action'] =='qc_post_document_in_process_pupup_ajax')
{
    ...
    $API=$INPROCESSQCPOSTDOCUMENTDETAILS.'?DocEntry='.$_POST['DocEntry'];
...
if(isset($_POST['SampleCollectionInProcess_Btn']))
{
    ...
    $tdata['Series']=trim(addslashes(strip_tags($_POST['IP_SC_DocNo'])));
...
{  
	...
	// echo "</pre>";
	// exit;
 	$tdata['Series']=trim(addslashes(strip_tags($_POST['QC_CK_D_DocNo'])));
	...
	$tdata['U_PC_MBy']=trim(addslashes(strip_tags($_POST['QC_CK_D_MfgBy'])));
	...
	$tdata['U_QCTType']=trim(addslashes(strip_tags($_POST['QC_CK_D_QCTesttype'])));
	...
	$ganaralData=array();
		...
		for ($i=0; $i <count($_POST['parameter_code']) ; $i++) { 
			$ganaralData['LineId']=trim(addslashes(strip_tags($_POST['QC_CK_D_LineNum'])));
		...
		for ($j=0; $j <count($_POST['qc_Status']) ; $j++) { 
            $qcStatus['LineId']=trim(addslashes(strip_tags($_POST['QC_CK_D_LineNum'][$j])));
...
{	
	...
	// exit;
    // $tdata['DocType']= "dDocument_Items";
    $tdata['Series']=trim(addslashes(strip_tags($_POST['qc_check_series'])));
...
{  
    ...
    // $tdata['Series']=trim(addslashes(strip_tags($_POST['qc_post_doc_Routestage_DocNo'])));
	$tdata['Object']=trim(addslashes(strip_tags('SCS_QCRSTAGE')));
    ...
    // trim(addslashes(strip_tags($_POST['routStage_WODate'])))
	...
	$ganaralData=array();
       ...
       if($_POST['qc_post_doc_Routestage_SampleType']==""){
        $data['status']='False';
		$data['DocEntry']='';
		$data['message']='Sample Type is required.';
		echo json_encode($data);
    ...
    if($_POST['qc_post_doc_Routestage_PostingDate']==""){
        $data['status']='False';
		$data['DocEntry']='';
		$data['message']='Posting Date is required.';
		echo json_encode($data);
    ...
    if($_POST['qc_post_doc_Routestage_AnalysisDate']==""){
        $data['status']='False';
		$data['DocEntry']='';
		$data['message']='Analysis Date is required.';
		echo json_encode($data);
    ...
    if($_POST['qc_post_doc_Routestage_ValidUpTo']==""){
        $data['status']='False';
		$data['DocEntry']='';
		$data['message']='ValidUpTo Date is required.';
		echo json_encode($data);
